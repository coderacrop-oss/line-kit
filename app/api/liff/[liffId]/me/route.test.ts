...
describe('GET /api/liff/[liffId]/me', () => {
  ...
  it('a server-to-server caller with a valid API key and X-Line-User-Id header succeeds', async () => {
    vi.mocked(resolveLiffParticipant).mockResolvedValue({
      ok: true, participantId: 'participant-9',
      liffApp: {
        id: 'app-1', name: 'Test', liffId: 'liff-1', lineLoginChannelId: '2011037337',
        channelId: 'channel-1', apiKeyLast4: 'abcd', createdAt: new Date(),
      },
    ...
    const request = new Request('https://example.com/api/liff/liff-1/me', {
      headers: { Authorization: 'Bearer sk_valid', 'X-Line-User-Id': 'U-server' },
    })
    const response = await GET(request, { params: Promise.resolve({ liffId: 'liff-1' }) })
    expect(response.status).toBe(200)
    expect(await response.json()).toEqual({ participantId: 'participant-9' })
    // พิสูจน์ที่ระดับ route ว่า request (พร้อม header) ถูกส่งต่อให้ resolveLiffParticipant
    // จริง ไม่ใช่แค่ auth.ts เพียวๆ ที่ทดสอบแยกไว้แล้วใน auth.test.ts
    const calledRequest = vi.mocked(resolveLiffParticipant).mock.calls[0][2] as Request
    expect(calledRequest.headers.get('X-Line-User-Id')).toBe('U-server')
  })
