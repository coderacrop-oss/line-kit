...
describe('GET /api/imagemap/[cardId]/[width]', () => {
  it.each(['239', '241', '0', '-1', '1041', '1.5', 'abc', '1040px', ''])(
    ...
    async (width) => {
      const res = await GET(new Request('http://x'), paramsFor('11111111-1111-1111-1111-111111111111', width))
  ...
  it.each(['missing-card', 'not-a-uuid', '../../etc/passwd', ''])(
    'cardId ที่รูปร่างไม่ใช่ UUID เลย — 404 สะอาดๆ ก่อนแตะฐานข้อมูลด้วยซ้ำ: %s',
    async (cardId) => {
      const res = await GET(new Request('http://x'), paramsFor(cardId, '1040'))
      expect(res.status).toBe(404)
      expect(resolveImagemapVariantAsset).not.toHaveBeenCalled()
    },
  )

  it('cardId เป็น UUID จริง (ตัวพิมพ์ใหญ่ก็ได้) — ผ่านด่านรูปร่างไปถึง resolveImagemapVariantAsset', async () => {
    ...
    const res = await GET(new Request('http://x'), paramsFor('11111111-1111-1111-1111-111111111111'.toUpperCase(), '1040'))
    expect(res.status).toBe(200)
    expect(resolveImagemapVariantAsset).toHaveBeenCalled()
  })
  ...
  it('ETag ต่างกันเมื่อ storage_path ต่างกัน (ไฟล์คนละเวอร์ชัน)', async () => {
    ...
    storeGet.mockResolvedValueOnce(new Uint8Array([1]))
    const res1 = await GET(new Request('http://x'), paramsFor('11111111-1111-1111-1111-111111111111', '1040'))

    resolveImagemapVariantAsset.mockResolvedValueOnce({ storagePath: 'uploads/x/v2.jpg', mimeType: 'image/jpeg' })
    storeGet.mockResolvedValueOnce(new Uint8Array([1]))
    const res2 = await GET(new Request('http://x'), paramsFor('11111111-1111-1111-1111-111111111111', '1040'))
