...
const sqlMock = Object.assign(vi.fn(), { json: (value: unknown) => value })
...
describe('saveQuizConfigAction', () => {
  ...
  it('saves a valid config', async () => {
    ...
    // สองแกน ไม่ใช่แกนเดียว — schema ของ Task 2 (lib/quiz/schema.ts) บังคับ
    // axes.min(2) ก้อนที่มีแกนเดียวไม่ผ่าน validation จริง แม้จะตั้งใจให้เป็นก้อน
    // "ที่ควรผ่าน" ก็ตาม
    const validConfig = {
      mode: 'solo',
