...
import { GroupConfig, QuizConfig, QuizReplies, TEMPLATE_SCHEMA_VERSION, TemplateConfig, TemplateCopy } from './schema'
...
describe('templateCopy', () => {
  ...
  /**
   * Finding 6 — customFlexJson must be an object shape now, not any unknown value.
   * renderKeywordCustom returns this verbatim as a FlexMessage.
   */
...
describe('TemplateConfig', () => {
  ...
  it('TEMPLATE_SCHEMA_VERSION is 1', () => {
    expect(TEMPLATE_SCHEMA_VERSION).toBe(1)
  })

  it('accepts a config with schemaVersion 1', () => {
    expect(TemplateConfig.safeParse({ schemaVersion: 1, quiz: soloBase }).success).toBe(true)
  })

  it('rejects a config with any other schemaVersion', () => {
    expect(TemplateConfig.safeParse({ schemaVersion: 2, quiz: soloBase }).success).toBe(false)
    expect(TemplateConfig.safeParse({ quiz: soloBase }).success).toBe(false)
  })

  it('rejects a config whose quiz block fails QuizConfig validation', () => {
    const broken = { ...soloBase, fallbackResultCode: 'NOPE' }
    expect(TemplateConfig.safeParse({ schemaVersion: 1, quiz: broken }).success).toBe(false)
  })
})
