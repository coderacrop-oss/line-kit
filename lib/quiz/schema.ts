...
export const QuizAxis = z.object({
  ...
  // ฟิลด์เสริมสี่ตัวด้านล่างนี้ไม่บังคับทั้งหมด — ควิซส่วนใหญ่ (label/poles สั้นๆ พอ) ไม่ต้อง
  // ใช้เลย มีไว้เผื่อแคมเปญที่ต้องการชื่อแกนแบบเต็ม/หลายภาษา หรือแกนที่ทำหน้าที่เป็น "ผลลัพธ์"
  // ของตัวเองด้วย (เช่น โหมด group ที่แต่ละแกนคือกลุ่ม/ทีมหนึ่ง ไม่ใช่สเปกตรัมสองขั้ว) — จอ
  // ตั้งค่า (QuizConfigForm.tsx) ซ่อนฟิลด์เหล่านี้ไว้เป็นค่าเริ่มต้น กดปุ่ม "+" ทีละอันถึงจะโชว์
  // ช่องกรอก กันไม่ให้แกนที่ไม่ต้องการฟิลด์เสริมต้องเห็นฟอร์มรกโดยไม่จำเป็น
  labelEn: z.string().max(40).optional(),
  body: z.string().max(400).optional(),
  short: z.string().max(60).optional(),
  // เป็น string ไม่ใช่ number โดยตั้งใจ — ใช้แค่จัดลำดับการแสดงผล (เช่น "01", "02") ไม่ใช่
  // ค่าที่เอาไปคำนวณอะไร เก็บเป็นข้อความเทียบกับ id/label ตัวอื่นในไฟล์นี้ที่ผู้ตั้งค่าพิมพ์เอง
